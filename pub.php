...
$search =  mysql_real_escape_string(strip_tags($_REQUEST['search']));
$year = date("Y"); 
if (!isset($search) && $search=='')
    //alert();
        $sql="SELECT * FROM pubs where year=".$year;
else 
   $sql="SELECT * FROM pubs WHERE CONCAT (title,authors,journals,year,pid,id) like '%".$search."%' COLLATE latin1_general_ci";
