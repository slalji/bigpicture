...
echo  $this->Html->script('/js/tiny_mce4/jquery.tinymce.min.js');
echo  $this->Html->script('/js/tiny_mce4/tinymce.min.js');
...
tinyMCE.init({
    ...
    plugins: [
         ...
         "code jbimages table contextmenu directionality emoticons paste textcolor "
   ],
   toolbar1: "undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | styleselect",
   toolbar2: "| jbimages | link unlink anchor | image media | forecolor backcolor  | print preview code ",
   image_advtab: true ,
                ...
                if ($scaffoldField == $primaryKey) {
                    ...
                    if (!$simple_view || ($simple_view && !in_array($scaffoldField, array('created', 'modified', 'updated', 'create_user_id', 'update_user_id')))) {
                        ...
                        if (!empty($field_type[$scaffoldField]))
                            if (in_array($field_type[$scaffoldField], array('date', 'datetime', 'enum'))) {
                               ...
                               // else
                                ...
                                if ($field_type[$scaffoldField] == 'enum')
                                    $input = str_replace('"text"', '"select"', $input);
